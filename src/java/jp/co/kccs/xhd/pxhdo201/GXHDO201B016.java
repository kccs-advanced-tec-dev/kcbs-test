...
import jp.co.kccs.xhd.model.GXHDO201B016Model;
...
@Named
@ViewScoped
public class GXHDO201B016 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B016.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B016Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B016Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B016_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B016_display_page_count", session));
        }
    ...
    public void checkInputAndSearch() {
        ...
        // 2次脱脂号機
        if (!StringUtil.isEmpty(nijidasshigouki)) {
            // 2次脱脂号機が入力されている場合のみマスタチェックを実施
            boolean existGoukiMaster = false;
            try {
                QueryRunner queryRunnerWip = new QueryRunner(dataSourceWip);
                if (validateUtil.existGokukimas(getNijidasshigouki(), queryRunnerWip)) {
            ...
            if (!existGoukiMaster) {
                FacesMessage message = 
                        new FacesMessage(FacesMessage.SEVERITY_ERROR, MessageUtil.getMessage("XHD-000011", "2次脱脂号機"), null);
                FacesContext.getCurrentInstance().addMessage(null, message);
                return;
            }
        }        
    ...
    public long selectListDataCount() {
        ...
        try {
            ...
            String sql = "SELECT COUNT(LOTNO) AS CNT "
                    ...
                    + "AND   (? IS NULL OR SYUURYOUNICHIJI <= ?) "
                    + "AND   (? IS NULL OR NIJIDASSHIGOUKI = ?)";
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B016Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B016Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b016.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "焼成・2次脱脂(ベルト)");

            // ダウンロードファイル名
            String downloadFileName = "焼成・2次脱脂(ベルト)_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
    ...
    private List<Object> createSearchParam() {
        ...
        String paramNijidasshigouki = StringUtil.blankToNull(getNijidasshigouki());
        ...
        params.addAll(Arrays.asList(paramNijidasshigouki, paramNijidasshigouki));
