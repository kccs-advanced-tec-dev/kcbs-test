...
import jp.co.kccs.xhd.model.GXHDO201B033Model;
...
@Named
@ViewScoped
public class GXHDO201B033 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B033.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B033Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B033Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B033_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B033_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B033Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B033Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b033.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "外部電極・外部電極洗浄(バレル)");

            // ダウンロードファイル名
            String downloadFileName = "外部電極・外部電極洗浄(バレル)_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
