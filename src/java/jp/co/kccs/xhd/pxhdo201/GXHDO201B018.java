...
import jp.co.kccs.xhd.model.GXHDO201B018Model;
...
@Named
@ViewScoped
public class GXHDO201B018 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B018.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B018Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B018Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B018_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B018_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B018Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B018Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b018.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "焼成・RHK焼成");

            // ダウンロードファイル名
            String downloadFileName = "焼成・RHK焼成_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
