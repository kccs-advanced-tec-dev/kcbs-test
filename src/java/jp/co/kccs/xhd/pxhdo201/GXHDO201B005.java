...
import jp.co.kccs.xhd.model.GXHDO201B005Model;
...
@Named
@ViewScoped
public class GXHDO201B005 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B005.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B005Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B005Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B005_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B005_display_page_count", session));
        }
    ...
    public void checkInputAndSearch() {
        ...
        // 印刷ロールNo
        if (existError(validateUtil.checkC103(getInsaturollno(), "印刷ﾛｰﾙNo", 10))) {
            return;
        }
        // 発泡シートNo
        if (existError(validateUtil.checkC103(getHapposheetno(), "発泡ｼｰﾄNo", 8))) {
            return;
        }
        ...
        if (listCountMax > 0 && count > listCountMax) {
            // 検索結果が上限件数以上の場合エラー終了
            FacesMessage message = 
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, MessageUtil.getMessage("XHD-000056", listCountMax), null);
            FacesContext.getCurrentInstance().addMessage(null, message);
            return;
        } 
    ...
    public void selectListData() {
        try {
            ...
            String sql = "SELECT CONCAT(IFNULL(T1.KOJYO, ''), IFNULL(T1.LOTNO, ''), IFNULL(T1.EDABAN, '')) AS LOTNO"
                    ...
                    + ", T1.TAKUTO"
                    + ", T1.BIKO1"
                    + ", CONCAT(IFNULL(T1.SKOJYO, ''), IFNULL(T1.SLOTNO, ''), IFNULL(T1.SEDABAN, '')) AS SLOTNO"
                    + ", T1.tapelotno"
                    + ", T1.taperollno1"
                    + ", T1.taperollno2"
                    + ", T1.taperollno3"
                    ...
                    + "AND   (? IS NULL OR T1.HAPPOSHEETNO = ?) "
            ...
            ResultSetHandler<List<GXHDO201B005Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B005Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b005.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "積層・RSUS");

            // ダウンロードファイル名
            String downloadFileName = "積層・RSUS_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
    ...
    private List<Object> createSearchParam() {
        ...
        String paramInsaturollno = StringUtil.blankToNull(getInsaturollno());
        String paramHapposheetno = StringUtil.blankToNull(getHapposheetno());
        ...
        params.addAll(Arrays.asList(paramInsaturollno, paramInsaturollno));
        params.addAll(Arrays.asList(paramHapposheetno, paramHapposheetno));
