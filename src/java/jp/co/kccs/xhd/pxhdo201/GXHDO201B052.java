...
import jp.co.kccs.xhd.model.GXHDO201B052Model;
...
@Named
@ViewScoped
public class GXHDO201B052 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B052.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B052Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B052Model> getListData() {
        return listData;
    }
    ...
    /**
     * 検索条件：号機
     * @param goki the bgoki to set
     */
    public void setBgoki(String goki) {
        this.bgoki = goki;
    }
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B052_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B052_display_page_count", session));
        }
    ...
    public void checkInputAndSearch() {
        ...
        // 研磨号機
        if(!StringUtil.isEmpty(getBgoki())){
            boolean existGoki = false;
            try {
                QueryRunner queryRunnerWip = new QueryRunner(dataSourceWip);
                if(validateUtil.existGokukimas(getBgoki(), queryRunnerWip)){
                    existGoki = true;
                }
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B052Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B052Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // TODO json作る
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b052.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "焼成・焼成前ﾊﾞﾚﾙ");

            // ダウンロードファイル名
            String downloadFileName = "焼成・焼成前ﾊﾞﾚﾙ_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
    ...
    private List<Object> createSearchParam() {
        ...
        String paramBgoki = StringUtil.blankToNull(getBgoki());
        ...
        params.addAll(Arrays.asList(paramBgoki, paramBgoki));
