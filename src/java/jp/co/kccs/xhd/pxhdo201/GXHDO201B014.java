...
import jp.co.kccs.xhd.model.GXHDO201B014Model;
...
@Named
@ViewScoped
public class GXHDO201B014 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B014.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B014Model> listData = null;
    ...
    /** 検索条件：脱脂種類：combox */
    private List<SelectItem> dasshisyuruiList;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B014Model> getListData() {
        return listData;
    }
    ...
    /**
     * 検索条件：脱脂種類
     * @return the dasshisyuruiList
     */
    public List<SelectItem> getDasshisyuruiList() {
        return dasshisyuruiList;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B014_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B014_display_page_count", session));
        }
    ...
    /*脱脂種類：combox*/
    public void dasshisyuruiListData() {
        dasshisyuruiList = new ArrayList<>();
        dasshisyuruiList.add(new SelectItem(""));
        dasshisyuruiList.add(new SelectItem("Air脱脂"));
        dasshisyuruiList.add(new SelectItem("窒素脱脂"));
    }
    ...
    public long selectListDataCount() {
        ...
        try {
            ...
            String sql = "SELECT COUNT(LOTNO) AS CNT "
                    ...
                    + "AND   (? IS NULL OR SYURYONICHIJI <= ?) "
                    + "AND   (? IS NULL OR YOBIKANGOKI = ?)"
                    + "AND   (? IS NULL OR DASSHISYURUI = ?)";
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B014Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B014Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b014.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "焼成・Air脱脂_窒素脱脂");

            // ダウンロードファイル名
            String downloadFileName = "焼成・Air脱脂_窒素脱脂_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
    ...
    private List<Object> createSearchParam() {
        ...
        String paramYobikangoki = StringUtil.blankToNull(getYobikangoki());
        String paramDasshisyurui = StringUtil.blankToNull(getDasshisyurui());
        ...
        params.addAll(Arrays.asList(paramYobikangoki, paramYobikangoki));
        params.addAll(Arrays.asList(paramDasshisyurui, paramDasshisyurui));
