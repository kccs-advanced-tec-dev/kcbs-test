...
import jp.co.kccs.xhd.model.GXHDO201B050Model;
...
@Named
@ViewScoped
public class GXHDO201B050 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B050.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B050Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B050Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B050_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B050_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            String sql = "SELECT CONCAT(IFNULL(kojyo, ''), IFNULL(lotno, ''), IFNULL(edaban, '')) AS LOTNO"
                    ...
                    + ", sayadumegouki"
                    + ", sayadumehouhou"
                    + ", (CASE WHEN sayasyurui = 0 THEN '未使用' WHEN sayasyurui = 1 THEN '使用' ELSE NULL END) AS sayasyurui"
            ...
            ResultSetHandler<List<GXHDO201B050Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B050Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // TODO json作る
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b050.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "焼成・真空脱脂ｻﾔ詰め");

            // ダウンロードファイル名
            String downloadFileName = "焼成・真空脱脂ｻﾔ詰め_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
