...
import jp.co.kccs.xhd.model.GXHDO201B032Model;
...
@Named
@ViewScoped
public class GXHDO201B032 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B032.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B032Model> listData = null;
    ...
    /** 検索条件：撥水処理日(FROM) */
    private String hassuiDateF = "";
    /** 検索条件：撥水処理日(TO) */
    private String hassuiDateT = "";
    /** 検索条件：撥水処理時刻(FROM) */
    private String hassuiTimeF = "";
    /** 検索条件：撥水処理時刻(TO) */
    private String hassuiTimeT = "";
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B032Model> getListData() {
        return listData;
    }
    ...
    /**
     * 検索条件：撥水処理担当者
     * @param hassuitantosyacode the hassuitantosyacode to set
     */
    ...
    /**
     * 検索条件：撥水処理日(FROM)
     * @return the hassuiDateF
     */
    public String getHassuiDateF() {
        return hassuiDateF;
    }

    /**
     * 検索条件：撥水処理日(FROM)
     * @param hassuiDateF the hassuiDateF to set
     */
    public void setHassuiDateF(String hassuiDateF) {
        this.hassuiDateF = hassuiDateF;
    }

    /**
     * 検索条件：撥水処理日(TO)
     * @return the hassuiDateT
     */
    public String getHassuiDateT() {
        return hassuiDateT;
    }

    /**
     * 検索条件：撥水処理日(TO)
     * @param hassuiDateT the hassuiDateT to set
     */
    public void setHassuiDateT(String hassuiDateT) {
        this.hassuiDateT = hassuiDateT;
    }

    /**
     * 検索条件：撥水処理時刻(FROM)
     * @return the hassuiTimeF
     */
    public String getHassuiTimeF() {
        return hassuiTimeF;
    }

    /**
     * 検索条件：撥水処理時刻(FROM)
     * @param hassuiTimeF the hassuiTimeF to set
     */
    public void setHassuiTimeF(String hassuiTimeF) {
        this.hassuiTimeF = hassuiTimeF;
    }

    /**
     * 検索条件：撥水処理時刻(TO)
     * @return the hassuiTimeT
     */
    public String getHassuiTimeT() {
        return hassuiTimeT;
    }

    /**
     * 検索条件：撥水処理時刻(TO)
     * @param hassuiTimeT the hassuiTimeT to set
     */
    public void setHassuiTimeT(String hassuiTimeT) {
        this.hassuiTimeT = hassuiTimeT;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B032_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B032_display_page_count", session));
        }
    ...
    public void checkInputAndSearch() {
        ...
        // 撥水処理日(FROM)
        if (existError(validateUtil.checkC101(getHassuiDateF(), "撥水処理日(from)", 6)) ||
            existError(validateUtil.checkC201ForDate(getHassuiDateF(), "撥水処理日(from)")) ||
            existError(validateUtil.checkC501(getHassuiDateF(), "撥水処理日(from)"))) {
            return;
        }
        // 撥水処理日(TO)
        if (existError(validateUtil.checkC101(getHassuiDateT(), "撥水処理日(to)", 6)) ||
            existError(validateUtil.checkC201ForDate(getHassuiDateT(), "撥水処理日(to)")) ||
            existError(validateUtil.checkC501(getHassuiDateT(), "撥水処理日(to)"))) {
            return;
        }
        // 撥水処理時刻(FROM)
        if (existError(validateUtil.checkC101(getHassuiTimeF(), "撥水処理時刻(from)", 4)) ||
            existError(validateUtil.checkC201ForDate(getHassuiTimeF(), "撥水処理時刻(from)")) ||
            existError(validateUtil.checkC502(getHassuiTimeF(), "撥水処理時刻(from)"))) {
            return;
        }
        if (!StringUtil.isEmpty(hassuiTimeF) && existError(validateUtil.checkC001(getHassuiDateF(), "撥水処理日(from)"))) {
            return;
        }
        // 撥水処理時刻(TO)
        if (existError(validateUtil.checkC101(getHassuiTimeT(), "撥水処理時刻(to)", 4)) ||
            existError(validateUtil.checkC201ForDate(getHassuiTimeT(), "撥水処理時刻(to)")) ||
            existError(validateUtil.checkC502(getHassuiTimeT(), "撥水処理時刻(to)"))) {
            return;
        }
        if (!StringUtil.isEmpty(hassuiTimeT) && existError(validateUtil.checkC001(getHassuiDateT(), "撥水処理日(to)"))) {
            return;
        }
        // KCPNO
        ...
        // 撥水処理担当者
        if (existError(validateUtil.checkC101(getHassuitantosyacode(), "撥水処理担当者", 6))) {
            return;
        }
        if(!StringUtil.isEmpty(getHassuitantosyacode())){
            boolean existGoki = false;
            try {
                QueryRunner queryRunnerWip = new QueryRunner(dataSourceWip);
                if(validateUtil.existTantomasEx(getHassuitantosyacode(), queryRunnerWip)){
                    existGoki = true;
                }
            ...
            if (!existGoki) {
              // 入力された担当者が[tantomas]に存在しない場合ｴﾗｰ。
                FacesMessage message = 
                        new FacesMessage(FacesMessage.SEVERITY_ERROR, MessageUtil.getMessage("XHD-000011", "撥水処理担当者"), null);
                FacesContext.getCurrentInstance().addMessage(null, message);
                return;  
            }
        }
    ...
    public long selectListDataCount() {
        ...
        try {
            ...
            String sql = "SELECT COUNT(LOTNO) AS CNT "
                    ...
                    + "AND   (? IS NULL OR HASSUIDATETIME >= ?) "
                    + "AND   (? IS NULL OR HASSUIDATETIME <= ?) "
                    + "AND   (? IS NULL OR HASSUITANTOSYACODE = ?)"
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B032Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B032Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b032.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "外部電極・外部電極洗浄(撥水処理)");

            // ダウンロードファイル名
            String downloadFileName = "外部電極・外部電極洗浄(撥水処理)_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
    ...
    private List<Object> createSearchParam() {
        ...
        Date paramHassuiDateF = null;
        if (!StringUtil.isEmpty(hassuiDateF)) {
            paramHassuiDateF = DateUtil.convertStringToDate(getHassuiDateF(), StringUtil.isEmpty(getHassuiTimeF()) ? "0000" : getHassuiTimeF());
        }
        Date paramHassuiDateT = null;
        if (!StringUtil.isEmpty(hassuiDateT)) {
            paramHassuiDateT = DateUtil.convertStringToDate(getHassuiDateT(), StringUtil.isEmpty(getHassuiTimeT()) ? "2359" : getHassuiTimeT());
        }
        String paramHassuitantosyacode = StringUtil.blankToNull(getHassuitantosyacode());
        ...
        params.addAll(Arrays.asList(paramHassuiDateF, paramHassuiDateF));
        params.addAll(Arrays.asList(paramHassuiDateT, paramHassuiDateT));
        params.addAll(Arrays.asList(paramHassuitantosyacode, paramHassuitantosyacode));
