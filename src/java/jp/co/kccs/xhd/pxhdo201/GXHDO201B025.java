...
import jp.co.kccs.xhd.model.GXHDO201B025Model;
...
@Named
@ViewScoped
public class GXHDO201B025 implements Serializable {

    private static final Logger LOGGER = Logger.getLogger(GXHDO201B025.class.getName());
     ...
     * 一覧表示データ
     */
    private List<GXHDO201B025Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B025Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B025_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B025_display_page_count", session));
        }
    ...
    public void checkInputAndSearch() {
        ...
        if (!StringUtil.isEmpty(gouki)) {
            ...
            if (!existGoukiMaster) {
                FacesMessage message
                        = new FacesMessage(FacesMessage.SEVERITY_ERROR, MessageUtil.getMessage("XHD-000011", "塗布号機"), null);
                FacesContext.getCurrentInstance().addMessage(null, message);
                return;
            }
        }
        ...
        // ﾍﾟｰｽﾄ品名
        if (existError(validateUtil.checkC103(getPastehinmei(), "ﾍﾟｰｽﾄ品名", 15))) {
            return;
        }
    ...
    public long selectListDataCount() {
        ...
        try {
            ...
            String sql = "SELECT COUNT(lotno) AS CNT "
                    ...
                    + "AND   (? IS NULL OR tofugoki = ?) "
                    + "AND   (? IS NULL OR pastehinmei = ?) ";
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B025Model>> beanHandler
                    = new BeanListHandler<>(GXHDO201B025Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b025.json"));
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "外部電極・外部電極塗布(三端子,4端子)");

            // ダウンロードファイル名
            String downloadFileName = "外部電極・外部電極塗布(三端子,4端子)_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
    ...
    private List<Object> createSearchParam() {
        ...
        String paramPastehinmei = StringUtil.blankToNull(getPastehinmei());
        ...
        params.addAll(Arrays.asList(paramPastehinmei, paramPastehinmei));
