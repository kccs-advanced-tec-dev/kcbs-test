...
import jp.co.kccs.xhd.model.GXHDO201B035Model;
...
@Named
@ViewScoped
public class GXHDO201B035 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B035.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B035Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B035Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B035_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B035_display_page_count", session));
        }
    ...
    public void checkInputAndSearch() {
        ...
        if(!StringUtil.isEmpty(getTantousya())){
            ...
            try {
                ...
                if(validateUtil.existTantomas(getTantousya(), queryRunnerWip)){
                    existGoki = true;
                }
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B035Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B035Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b035.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "外部電極・外部電極洗浄(乾燥)");

            // ダウンロードファイル名
            String downloadFileName = "外部電極・外部電極洗浄(乾燥)_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
