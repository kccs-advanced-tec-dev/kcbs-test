...
import jp.co.kccs.xhd.model.GXHDO201B019Model;
...
@Named
@ViewScoped
public class GXHDO201B019 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B019.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B019Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B019Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B019_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B019_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            String sql = "SELECT CONCAT(IFNULL(KOJYO, ''), IFNULL(LOTNO, ''), IFNULL(EDABAN, '')) AS LOTNO "
                    ...
                    + ", keepondo"
                    + ", (CASE WHEN atogaikan = 0 THEN 'NG' WHEN atogaikan = 1 THEN 'OK' ELSE NULL END) AS atogaikan "
                    + ", kaishusettasuu"
                    + ", kaisinichiji"
                    + ", StartTantosyacode"
                    + ", StartKakuninsyacode"
            ...
            mapping.put("siteisaisaka", "siteisaisaka");               // 指定再酸化回数
            mapping.put("tounyusettasuu", "tounyusettasuu");           // 投入セッタ枚数
            mapping.put("gouki", "gouki");                             // 号機
            mapping.put("setteipattern", "setteipattern");             // 設定ﾊﾟﾀｰﾝ
            mapping.put("keepondo", "keepondo");                       // ｷｰﾌﾟ温度
            mapping.put("atogaikan", "atogaikan");                     // 後外観確認
            mapping.put("kaishusettasuu", "kaishusettasuu");           // 回収セッタ枚数
            ...
            ResultSetHandler<List<GXHDO201B019Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B019Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b019.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "焼成・再酸化");

            // ダウンロードファイル名
            String downloadFileName = "焼成・再酸化_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
