 ...
 */
/**
 * GXHDO102C009(誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力)
 *
 * @author KCSS K.Jo
 * @since 2021/11/09
 */
@ManagedBean(name = "beanGXHDO102C009")
@SessionScoped
public class GXHDO102C009 implements Serializable {

    private static final Logger LOGGER = Logger.getLogger(GXHDO102C009.class.getName());
    ...
    /**
     * 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力サブ画面用データ
     */
    private GXHDO102C009Model gxhdO102c009Model;

    /**
     * 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力サブ画面用データ(表示制御用)
     */
    private GXHDO102C009Model gxhdO102c009ModelView;
    ...
    public void doOk() {
        ...
        GXHDO102C009Logic.setSubgamenDataFromView(this.getGxhdO102c009ModelView(), this.getGxhdO102c009Model());
    }
    ...
    private boolean checkOK() {
        ...
        GXHDO102C009Model.SubGamenData showsubgamendata = this.getGxhdO102c009ModelView().getShowsubgamendata();
        // 背景色をクリア
        GXHDO102C009Logic.clearBackColor(this.getGxhdO102c009ModelView());
        setErrorTabIndex(0);
        // サブ画面の部材①タブデータリスト
        ...
        FXHDD01 subDataTyogouryoukikaku = this.getGxhdO102c009ModelView().getShowsubgamendata().getSubDataTyogouryoukikaku();
    ...
    private boolean checkTyogouryouKikaku(List<FXHDD01> subDataBuzaitab1List, List<FXHDD01> subDataBuzaitab2List,FXHDD01 subDataTyogouryoukikaku) {
        ...
        String tyogouryou1Tab1 = subDataBuzaitab1List.get(4).getValue();
        String tyogouryou2Tab1 = subDataBuzaitab1List.get(5).getValue();
        String tyogouryou3Tab1 = subDataBuzaitab1List.get(6).getValue();
        String tyogouryou4Tab1 = subDataBuzaitab1List.get(7).getValue();
        String tyogouryou5Tab1 = subDataBuzaitab1List.get(8).getValue();
        String tyogouryou6Tab1 = subDataBuzaitab1List.get(9).getValue();

        String tyogouryou1Tab2 = subDataBuzaitab2List.get(4).getValue();
        String tyogouryou2Tab2 = subDataBuzaitab2List.get(5).getValue();
        String tyogouryou3Tab2 = subDataBuzaitab2List.get(6).getValue();
        String tyogouryou4Tab2 = subDataBuzaitab2List.get(7).getValue();
        String tyogouryou5Tab2 = subDataBuzaitab2List.get(8).getValue();
        String tyogouryou6Tab2 = subDataBuzaitab2List.get(9).getValue();
    ...
    /**
     * 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力_サブ画面の調合残量計算処理
     *
     */
    public void doTyogouzanryouKeisan() {
        //サブ画面の調合残量の計算処理
        GXHDO102C009Logic.calcTyogouzanryou(this.getGxhdO102c009ModelView());
        GXHDO102C009Logic.setShowsubgamendataFromView(this.getGxhdO102c009ModelView(), this.getGxhdO102c009Model());
    }
    ...
    private boolean checkBuzaitab(List<FXHDD01> subDataBuzaitabList) {
        ...
        if (!tyogouryouCheck(subDataBuzaitabList.get(4), subDataBuzaitabList.get(5))) {
            return false;
        }
        // 調合量X_4に調合量が入力されている場合、調合量X_3に0が入力されていること。
        if (!tyogouryouCheck(subDataBuzaitabList.get(6), subDataBuzaitabList.get(7))) {
            return false;
        }
        // 調合量X_6に調合量が入力されている場合、調合量X_5に0が入力されていること。
        return tyogouryouCheck(subDataBuzaitabList.get(8), subDataBuzaitabList.get(9));
    }
    ...
    private boolean buzainoCheck(FXHDD01 itemBuzaino, String subTabNo) {
        // 部材在庫品名の値をクリア
        setBuzaihinmeiVal(this.getGxhdO102c009ModelView(), subTabNo, "");
        ...
        setBuzaihinmeiVal(this.getGxhdO102c009ModelView(), subTabNo, hinmei);
     ...
     * @param hinmei 在庫品名
     */
    private void setBuzaihinmeiVal(GXHDO102C009Model gxhdo102c009model, String subTabNo, String hinmei) {
     ...
     * エラーセット
     *
     * @param itemData 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力サブ画面
     * @param errorId エラーID
     ...
     * 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力サブ画面用データ
     *
     * @return the gxhdO102c009Model
     */
    public GXHDO102C009Model getGxhdO102c009Model() {
     ...
     * 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力サブ画面用データ
     *
     * @param gxhdO102c009Model the gxhdO102c009Model to set
     */
    public void setGxhdO102c009Model(GXHDO102C009Model gxhdO102c009Model) {
        this.gxhdO102c009Model = gxhdO102c009Model;
    }

    /**
     * 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力サブ画面用データ(表示制御用)
     *
     * @return the gxhdO102c009ModelView
     */
    public GXHDO102C009Model getGxhdO102c009ModelView() {
        return gxhdO102c009ModelView;
    }

    /**
     * 誘電体ｽﾗﾘｰ作製・溶剤・添加材ｽﾗﾘｰ秤量入力サブ画面用データ(表示制御用)
     *
     * @param gxhdO102c009ModelView the gxhdO102c009ModelView to set
     */
    public void setGxhdO102c009ModelView(GXHDO102C009Model gxhdO102c009ModelView) {
        this.gxhdO102c009ModelView = gxhdO102c009ModelView;
    }
}
