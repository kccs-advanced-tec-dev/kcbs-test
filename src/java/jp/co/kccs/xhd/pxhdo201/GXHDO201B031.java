...
import jp.co.kccs.xhd.model.GXHDO201B031Model;
...
@Named
@ViewScoped
public class GXHDO201B031 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B031.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B031Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B031Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B031_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B031_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B031Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B031Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b031.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "外部電極・外部電極焼成(樹脂電極硬化)");

            // ダウンロードファイル名
            String downloadFileName = "外部電極・外部電極焼成(樹脂電極硬化)_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
