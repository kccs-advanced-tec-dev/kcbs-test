...
import jp.co.kccs.xhd.model.GXHDO201B048Model;
...
@Named
@ViewScoped
public class GXHDO201B048 implements Serializable {

    private static final Logger LOGGER = Logger.getLogger(GXHDO201B048.class.getName());
     ...
     * 一覧表示データ
     */
    private List<GXHDO201B048Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B048Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B048_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B048_display_page_count", session));
        }
    ...
    private void settingError() {
        ...
        messageList.add(MessageUtil.getMessage("XHD-000185"));
...
//        }
    ...
    public void selectListData() {
        ...
        try {
            ...
            String sql = "SELECT "
                    ...
                    + ", tounyuusu"
                    + ", ryouhintopreelmaki1"
                    + ", ryouhintopreelhonsu1"
                    + ", ryouhintopreelmaki2"
                    + ", ryouhintopreelhonsu2"
                    + ", ryouhinsu"
                    + ", youryong1"
                    + ", gazoungue2"
                    + ", gazoungsita2"
            ...
            ResultSetHandler<List<GXHDO201B048Model>> beanHandler
                    = new BeanListHandler<>(GXHDO201B048Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b048.json"));
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "テーピング作業");

            // ダウンロードファイル名
            String downloadFileName = "テーピング作業_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
    ...
    private Map loadFxhbm03Data(List<String> userGrpList) {
        try {
            ...
            sql = sql + " AND key = 'ﾃｰﾋﾟﾝｸﾞ作業履歴_表示可能ﾃﾞｰﾀ'";
