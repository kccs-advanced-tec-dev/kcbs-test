...
import jp.co.kccs.xhd.model.GXHDO201B011Model;
...
@Named
@ViewScoped
public class GXHDO201B011 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B011.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B011Model> listData = null;
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B011Model> getListData() {
        return listData;
    }
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B011_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B011_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            mapping.put("atumimin", "atumimin");                  //生T寸 Min(μm)            
            mapping.put("atumimax", "atumimax");                  //生T寸 Max(μm)           
            mapping.put("cutbashurui", "cutbashurui");            //ｶｯﾄ刃種類(μm)            
            mapping.put("cutmuki", "cutmuki");                    //ｶｯﾄ向き            
            mapping.put("happosheetcolor", "happosheetcolor");    //発泡ｼｰﾄ色            
            mapping.put("kansou", "kansou");                      //乾燥(min)  
            ...
            ResultSetHandler<List<GXHDO201B011Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B011Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b011.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "カット・ダイシングカット");

            // ダウンロードファイル名
            String downloadFileName = "カット・ダイシングカット_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
