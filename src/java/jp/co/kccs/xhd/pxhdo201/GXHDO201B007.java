...
import jp.co.kccs.xhd.model.GXHDO201B007Model;
...
@Named
@ViewScoped
public class GXHDO201B007 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B007.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B007Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B007Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B007_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B007_display_page_count", session));
        }
    ...
    public void checkInputAndSearch() {
        ...
        if(!StringUtil.isEmpty(getGoki())){
            ...
            if (!existGoki) {
              // 入力された仮ﾌﾟﾚｽ号機が[gokukimas.goukicode]に存在しない場合ｴﾗｰ。
            FacesMessage message = 
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, MessageUtil.getMessage("XHD-000011", "仮ﾌﾟﾚｽ号機"), null);
            FacesContext.getCurrentInstance().addMessage(null, message);
            return;  
            }
        }
    ...
    public void selectListData() {
        try {
            ...
            String sql = "SELECT CONCAT(IFNULL(KOJYO, ''), IFNULL(LOTNO, ''), IFNULL(EDABAN, '')) AS LOTNO"
                    ...
                    + ", jissekino"
                    + ", koteicode"
                    + ", (CASE WHEN kansyouzai = 0 THEN 'なし' WHEN kansyouzai = 1 THEN 'あり' ELSE '' END) AS kansyouzai"
                    + ", shinkuuhoji"
                    + ", aturyokusetteiti2"
                    + ", kaatujikan2"
                    + ", aturyokusetteiti3"
            ...
            mapping.put("startdatetime", "kaisinichiji");             // 開始日時
            mapping.put("enddatetime", "syuryonichiji");              // 終了日時
            mapping.put("ondohyoji", "ondohyoji");                    // 温度(℃)
            mapping.put("aturyokusetteiti", "aturyokusetteiti");      // 1次圧力(KN)
            mapping.put("kaatujikan", "kaatujikan");                  // 1次時間(s)
            mapping.put("goki", "goki");                              // 仮ﾌﾟﾚｽ号機
            mapping.put("tantosya", "kaisitantosya");                 // 開始担当者
            mapping.put("kakuninsya", "kaisikakusya");                // 開始確認者
            mapping.put("biko1", "biko1");                            // 備考1
            mapping.put("sagyokubun", "sagyokubun");                  // 作業区分
            mapping.put("setsuu", "setsuu");                          // 処理ｾｯﾄ数(SET)
            mapping.put("tansimaisuu", "tansimaisuu");                // 端子枚数(枚)
            ...
            mapping.put("shinkuuhoji", "shinkuuhoji");                // 真空保持(min)
            mapping.put("aturyokusetteiti2", "aturyokusetteiti2");    // 2次圧力(KN)
            mapping.put("kaatujikan2", "kaatujikan2");                // 2次時間(s)
            mapping.put("aturyokusetteiti3", "aturyokusetteiti3");    // 3次圧力(KN)
            mapping.put("kaatujikan3", "kaatujikan3");                // 3次時間(s)
            mapping.put("shinkuudo", "shinkuudo");                    // 真空度
            mapping.put("Hokanjouken", "hokanjouken");                // 保管条件
            mapping.put("EndTantousyacode", "syuryotantosya");        // 終了担当者
            mapping.put("RyouhinSetsuu", "ryouhinSetsuu");            // 良品ｾｯﾄ数(SET)
            mapping.put("biko2", "biko2");                            // 備考2
            
            BeanProcessor beanProcessor = new BeanProcessor(mapping);
            RowProcessor rowProcessor = new BasicRowProcessor(beanProcessor);
            ResultSetHandler<List<GXHDO201B007Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B007Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b007.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "プレス・仮プレス");

            // ダウンロードファイル名
            String downloadFileName = "プレス・仮プレス_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
