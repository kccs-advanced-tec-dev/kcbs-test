 ...
 */
/**
 * GXHDO101B017(焼成・焼成)ロジック
 *
 * @author SYSNAVI K.Hisanaga
 * @since 2019/06/15
 */
public class GXHDO101B017 implements IFormLogic {
    ...
    public ProcessData initial(ProcessData processData) {
        ...
        try {
            ...
            processData.setNoCheckButtonId(Arrays.asList(
                    GXHDO101B017Const.BTN_STARTDATETIME_TOP,
                    GXHDO101B017Const.BTN_STARTDATETIME_BOTTOM,
                    GXHDO101B017Const.BTN_ENDDATETIME_TOP,
                    GXHDO101B017Const.BTN_ENDDATETIME_BOTTOM
            ));

            // リビジョンチェック対象のボタンを設定する。
            processData.setCheckRevisionButtonId(Arrays.asList(
                    GXHDO101B017Const.BTN_KARI_TOUROKU_TOP,
                    GXHDO101B017Const.BTN_KARI_TOUROKU_BOTTOM,
                    GXHDO101B017Const.BTN_INSERT_TOP,
                    GXHDO101B017Const.BTN_INSERT_BOTTOM,
                    GXHDO101B017Const.BTN_DELETE_TOP,
                    GXHDO101B017Const.BTN_DELETE_BOTTOM,
                    GXHDO101B017Const.BTN_UPDATE_TOP,
                    GXHDO101B017Const.BTN_UPDATE_BOTTOM));
    ...
    private ErrorMessageInfo checkItemRegistCorrect(ProcessData processData) {
        ...
        FXHDD01 shinkuCheck = getItemRow(processData.getItemList(), GXHDO101B017Const.SYOSEI_SETTEI_PTN_CHECK); //焼成設定ﾊﾟﾀｰﾝﾁｪｯｸ
        ...
        // 開始日時、終了日時前後チェック
        FXHDD01 kaishiDay = getItemRow(processData.getItemList(), GXHDO101B017Const.KAISHI_DAY); //開始日
        FXHDD01 kaishiTime = getItemRow(processData.getItemList(), GXHDO101B017Const.KAISHI_TIME); //開始時刻
        Date kaishiDate = DateUtil.convertStringToDate(kaishiDay.getValue(), kaishiTime.getValue());
        FXHDD01 shuryouDay = getItemRow(processData.getItemList(), GXHDO101B017Const.SHURYOU_DAY); //終了日
        FXHDD01 shuryouTime = getItemRow(processData.getItemList(), GXHDO101B017Const.SHURYOU_TIME); //終了時刻
    ...
    public ProcessData checkDataCorrect(ProcessData processData) {
        ...
        processData.setUserAuthParam(GXHDO101B017Const.USER_AUTH_UPDATE_PARAM);
    ...
    public ProcessData checkDataDelete(ProcessData processData) {
        ...
        // ユーザ認証用のパラメータをセットする。
        processData.setRquireAuth(true);
        processData.setUserAuthParam(GXHDO101B017Const.USER_AUTH_DELETE_PARAM);
        ...
        switch (jotaiFlg) {
            case JOTAI_FLG_TOROKUZUMI:
                activeIdList.addAll(Arrays.asList(
                        ...
                        GXHDO101B017Const.BTN_ENDDATETIME_TOP,
                        GXHDO101B017Const.BTN_ENDDATETIME_BOTTOM,
                        GXHDO101B017Const.BTN_COPY_EDABAN_TOP,
                        GXHDO101B017Const.BTN_COPY_EDABAN_BOTTOM,
                        GXHDO101B017Const.BTN_UPDATE_TOP,
                        GXHDO101B017Const.BTN_UPDATE_BOTTOM,
                        GXHDO101B017Const.BTN_DELETE_TOP,
                        GXHDO101B017Const.BTN_DELETE_BOTTOM
    ...
    public String convertButtonIdToMethod(String buttonId) {
        ...
        switch (buttonId) {
            // 仮登録
            case GXHDO101B017Const.BTN_KARI_TOUROKU_TOP:
            case GXHDO101B017Const.BTN_KARI_TOUROKU_BOTTOM:
                method = "checkDataTempRegist";
                break;
            // 登録
            case GXHDO101B017Const.BTN_INSERT_TOP:
            case GXHDO101B017Const.BTN_INSERT_BOTTOM:
                method = "checkDataRegist";
                break;
            // 枝番コピー
            case GXHDO101B017Const.BTN_COPY_EDABAN_TOP:
            case GXHDO101B017Const.BTN_COPY_EDABAN_BOTTOM:
                method = "confEdabanCopy";
                break;
            // 修正
            case GXHDO101B017Const.BTN_UPDATE_TOP:
            case GXHDO101B017Const.BTN_UPDATE_BOTTOM:
                method = "checkDataCorrect";
                break;
            // 削除
            case GXHDO101B017Const.BTN_DELETE_TOP:
            case GXHDO101B017Const.BTN_DELETE_BOTTOM:
                method = "checkDataDelete";
                break;
            // 開始日時
            case GXHDO101B017Const.BTN_STARTDATETIME_TOP:
            case GXHDO101B017Const.BTN_STARTDATETIME_BOTTOM:
                method = "setStartDateTime";
                break;
            // 終了日時
            case GXHDO101B017Const.BTN_ENDDATETIME_TOP:
            case GXHDO101B017Const.BTN_ENDDATETIME_BOTTOM:
    ...
    private void setViewItemData(ProcessData processData, Map lotKbnMasData, Map ownerMasData, Map shikakariData, String lotNo) {
        ...
        // ロットNo
        this.setItemData(processData, GXHDO101B017Const.LOTNO, lotNo);
        // KCPNO
        this.setItemData(processData, GXHDO101B017Const.KCPNO, StringUtil.nullToBlank(getMapData(shikakariData, "kcpno")));
        // 客先
        this.setItemData(processData, GXHDO101B017Const.KYAKUSAKI, StringUtil.nullToBlank(getMapData(shikakariData, "tokuisaki")));

        // ロット区分
        String lotkubuncode = StringUtil.nullToBlank(getMapData(shikakariData, "lotkubuncode")); //ﾛｯﾄ区分ｺｰﾄﾞ
        if (StringUtil.isEmpty(lotkubuncode)) {
            this.setItemData(processData, GXHDO101B017Const.LOT_KUBUN, "");
        } else {
            String lotKubun = StringUtil.nullToBlank(getMapData(lotKbnMasData, "lotkubun"));
            this.setItemData(processData, GXHDO101B017Const.LOT_KUBUN, lotkubuncode + ":" + lotKubun);
        }

        // オーナー
        String ownercode = StringUtil.nullToBlank(getMapData(shikakariData, "ownercode"));// ｵｰﾅｰｺｰﾄﾞ
        if (StringUtil.isEmpty(lotkubuncode)) {
            this.setItemData(processData, GXHDO101B017Const.OWNER, "");
        } else {
            String owner = StringUtil.nullToBlank(getMapData(ownerMasData, "ownername"));
            this.setItemData(processData, GXHDO101B017Const.OWNER, ownercode + ":" + owner);
        }

        // 指示
        this.setItemData(processData, GXHDO101B017Const.SIJI, "");
    ...
    private boolean setInputItemData(ProcessData processData, QueryRunner queryRunnerDoc, QueryRunner queryRunnerQcdb,
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (StringUtil.isEmpty(rev) || !(JOTAI_FLG_KARI_TOROKU.equals(jotaiFlg) || JOTAI_FLG_TOROKUZUMI.equals(jotaiFlg))) {
                ...
                FXHDD01 itemSettaMaisu = this.getItemRow(processData.getItemList(), GXHDO101B017Const.UKEIRE_SETTA_MAISU);
    ...
    private void setInputItemDataMainForm(ProcessData processData, SrSyosei srSyoseiData) {
        ...
        //受入ｾｯﾀ枚数
        this.setItemData(processData, GXHDO101B017Const.UKEIRE_SETTA_MAISU, getSrSyoseiItemData(GXHDO101B017Const.UKEIRE_SETTA_MAISU, srSyoseiData));
        //投入ｾｯﾀ枚数
        this.setItemData(processData, GXHDO101B017Const.TOUNYU_SETTA_MAISU, getSrSyoseiItemData(GXHDO101B017Const.TOUNYU_SETTA_MAISU, srSyoseiData));
        //焼成号機
        this.setItemData(processData, GXHDO101B017Const.SYOSEI_GOUKI, getSrSyoseiItemData(GXHDO101B017Const.SYOSEI_GOUKI, srSyoseiData));
        //焼成設定ﾊﾟﾀｰﾝﾁｪｯｸ
        this.setItemData(processData, GXHDO101B017Const.SYOSEI_SETTEI_PTN_CHECK, getSrSyoseiItemData(GXHDO101B017Const.SYOSEI_SETTEI_PTN_CHECK, srSyoseiData));
        //焼成ﾋﾟｰｸ温度
        this.setItemData(processData, GXHDO101B017Const.SYOSEI_PEAK_ONDO, getSrSyoseiItemData(GXHDO101B017Const.SYOSEI_PEAK_ONDO, srSyoseiData));
        //段数
        this.setItemData(processData, GXHDO101B017Const.DANSU, getSrSyoseiItemData(GXHDO101B017Const.DANSU, srSyoseiData));
        //外観確認
        this.setItemData(processData, GXHDO101B017Const.GAIKAN_KAKUNIN, getSrSyoseiItemData(GXHDO101B017Const.GAIKAN_KAKUNIN, srSyoseiData));
        //回収ｾｯﾀ枚数
        this.setItemData(processData, GXHDO101B017Const.KAISHU_SETTA_MAISU, getSrSyoseiItemData(GXHDO101B017Const.KAISHU_SETTA_MAISU, srSyoseiData));
        //開始日
        this.setItemData(processData, GXHDO101B017Const.KAISHI_DAY, getSrSyoseiItemData(GXHDO101B017Const.KAISHI_DAY, srSyoseiData));
        //開始時間
        this.setItemData(processData, GXHDO101B017Const.KAISHI_TIME, getSrSyoseiItemData(GXHDO101B017Const.KAISHI_TIME, srSyoseiData));
        //開始担当者
        this.setItemData(processData, GXHDO101B017Const.KAISHI_TANTOUSYA, getSrSyoseiItemData(GXHDO101B017Const.KAISHI_TANTOUSYA, srSyoseiData));
        //開始確認者
        this.setItemData(processData, GXHDO101B017Const.KAISHI_KAKUNINSYA, getSrSyoseiItemData(GXHDO101B017Const.KAISHI_KAKUNINSYA, srSyoseiData));
        //終了日
        this.setItemData(processData, GXHDO101B017Const.SHURYOU_DAY, getSrSyoseiItemData(GXHDO101B017Const.SHURYOU_DAY, srSyoseiData));
        //終了時間
        this.setItemData(processData, GXHDO101B017Const.SHURYOU_TIME, getSrSyoseiItemData(GXHDO101B017Const.SHURYOU_TIME, srSyoseiData));
        //終了担当者
        this.setItemData(processData, GXHDO101B017Const.SHURYOU_TANTOUSYA, getSrSyoseiItemData(GXHDO101B017Const.SHURYOU_TANTOUSYA, srSyoseiData));
        //備考1
        this.setItemData(processData, GXHDO101B017Const.BIKOU1, getSrSyoseiItemData(GXHDO101B017Const.BIKOU1, srSyoseiData));
        //備考2
        this.setItemData(processData, GXHDO101B017Const.BIKOU2, getSrSyoseiItemData(GXHDO101B017Const.BIKOU2, srSyoseiData));
    }
    ...
    private void updateTmpSrSyosei(QueryRunner queryRunnerQcdb, Connection conQcdb, BigDecimal rev, String jotaiFlg, BigDecimal newRev,
        ...
        String sql = "UPDATE tmp_sr_syosei SET "
                ...
                + "biko1 = ?,biko2 = ?,kosinnichiji = ?,tounyusettasuu = ?,setteipattern = ?,dansuu = ?,gaikancheck = ?,kaishusettasuu = ?,"
                + "StartKakuninsyacode = ?,syoseisyurui = ?,revision = ?,deleteflag = ? "
    ...
    private List<Object> setUpdateParameterTmpSrSyosei(boolean isInsert, BigDecimal newRev, int deleteflag, String kojyo,
        ...
        params.add(DBUtil.stringToStringObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.KCPNO, srSyoseiData)));  //KCPNO
        if (isInsert) {
            params.add(null); //個数
        ...
        params.add(DBUtil.stringToDateObjectDefaultNull(
                getItemData(itemList, GXHDO101B017Const.KAISHI_DAY, srSyoseiData),
                getItemData(itemList, GXHDO101B017Const.KAISHI_TIME, srSyoseiData))); //焼成開始日時
        params.add(DBUtil.stringToDateObjectDefaultNull(
                getItemData(itemList, GXHDO101B017Const.SHURYOU_DAY, srSyoseiData),
                getItemData(itemList, GXHDO101B017Const.SHURYOU_TIME, srSyoseiData))); //焼成終了日時
        if (isInsert) {
            params.add(null); //ﾊﾞｯﾁ炉ﾌﾟﾛｸﾞﾗﾑNo
        }
        params.add(DBUtil.stringToIntObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.SYOSEI_PEAK_ONDO, srSyoseiData)));  //焼成温度
        params.add(DBUtil.stringToStringObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.SYOSEI_GOUKI, srSyoseiData)));  //ﾄﾝﾈﾙ炉・ﾊﾞｯﾁ炉号機
        params.add(DBUtil.stringToIntObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.UKEIRE_SETTA_MAISU, srSyoseiData)));  //受入ｾｯﾀ枚数
        if (isInsert) {
            params.add(null); //入炉台板枚数
        }
        params.add(DBUtil.stringToStringObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.KAISHI_TANTOUSYA, srSyoseiData)));  //焼成開始担当者
        params.add(DBUtil.stringToStringObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.SHURYOU_TANTOUSYA, srSyoseiData)));  //終了担当者
        params.add(DBUtil.stringToStringObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.BIKOU1, srSyoseiData)));  //備考1
        params.add(DBUtil.stringToStringObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.BIKOU2, srSyoseiData)));  //備考2
        if (isInsert) {
            params.add(null); //備考3
            params.add(null); //備考4
            params.add(null); //備考5
        ...
        params.add(DBUtil.stringToIntObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.TOUNYU_SETTA_MAISU, srSyoseiData)));  //投入ｾｯﾀ枚数
        params.add(getCheckBoxDbValue(getItemData(itemList, GXHDO101B017Const.SYOSEI_SETTEI_PTN_CHECK, srSyoseiData), null));  //焼成設定ﾊﾟﾀｰﾝﾁｪｯｸ
        params.add(DBUtil.stringToIntObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.DANSU, srSyoseiData)));  //段数
        params.add(getComboValue(getItemData(itemList, GXHDO101B017Const.GAIKAN_KAKUNIN, srSyoseiData), null));//外観確認
        params.add(DBUtil.stringToIntObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.KAISHU_SETTA_MAISU, srSyoseiData)));  //回収ｾｯﾀ枚数
        params.add(DBUtil.stringToStringObjectDefaultNull(getItemData(itemList, GXHDO101B017Const.KAISHI_KAKUNINSYA, srSyoseiData)));  //開始確認者
        if (isInsert) {
            params.add(null); //2次脱脂号機
            params.add(null); //2次脱脂設定ﾊﾟﾀｰﾝ
            params.add(null); //2次脱脂ｷｰﾌﾟ温度
            params.add(null); //2次脱脂ｺﾝﾍﾞｱ速度
            params.add(null); //焼成ﾋﾟｰｸ温度指示
            params.add(null); //焼成ﾛｰﾗｰ速度
            params.add(null); //焼成ﾊﾟｰｼﾞ
            params.add(null); //1回目再酸化号機1
            params.add(null); //1回目再酸化号機2
            params.add(null); //1回目再酸化設定ﾊﾟﾀｰﾝ
            params.add(null); //1回目再酸化ｷｰﾌﾟ温度
            params.add(null); //1回目再酸化ｺﾝﾍﾞｱ速度
            params.add(null); //1回目再酸化後外観
        }
        params.add("焼成"); //焼成種類
    ...
    private void updateSrSyosei(QueryRunner queryRunnerQcdb, Connection conQcdb, BigDecimal rev, String jotaiFlg, BigDecimal newRev,
        ...
        String sql = "UPDATE sr_syosei SET "
                + "kcpno = ?,skaisinichiji = ?,ssyuryonichiji = ?,syoseiondo = ?,goki = ?,ssettermaisuu = ?,skaisitantosya = ?,ssyuryotantosya=?,"
                + "biko1 = ?,biko2 = ?,kosinnichiji = ?,tounyusettasuu = ?,setteipattern = ?,dansuu = ?,gaikancheck = ?,kaishusettasuu = ?,"
                + "StartKakuninsyacode = ?,syoseisyurui = ?,revision = ? "
                + "WHERE kojyo = ? AND lotno = ? AND edaban = ? AND revision = ? AND jissekino = ? ";
    ...
    private List<Object> setUpdateParameterSrSyosei(boolean isInsert, BigDecimal newRev, String kojyo, String lotNo, String edaban, int jissekino,
        ...
        params.add(DBUtil.stringToStringObject(getItemData(itemList, GXHDO101B017Const.KCPNO, srSyoseiData)));  //KCPNO
        if (isInsert) {
            params.add(0); //個数
            params.add(""); //原料品種名
            params.add(""); //原料ｸﾞﾙｰﾌﾟ
        ...
        params.add(DBUtil.stringToIntObject(getItemData(itemList, GXHDO101B017Const.SYOSEI_PEAK_ONDO, srSyoseiData)));  //焼成温度
        params.add(DBUtil.stringToStringObject(getItemData(itemList, GXHDO101B017Const.SYOSEI_GOUKI, srSyoseiData)));  //ﾄﾝﾈﾙ炉・ﾊﾞｯﾁ炉号機
        params.add(DBUtil.stringToIntObject(getItemData(itemList, GXHDO101B017Const.UKEIRE_SETTA_MAISU, srSyoseiData)));  //受入ｾｯﾀ枚数
        if (isInsert) {
            params.add(0); //入炉台板枚数
        }
        params.add(DBUtil.stringToStringObject(getItemData(itemList, GXHDO101B017Const.KAISHI_TANTOUSYA, srSyoseiData)));  //焼成開始担当者
        params.add(DBUtil.stringToStringObject(getItemData(itemList, GXHDO101B017Const.SHURYOU_TANTOUSYA, srSyoseiData)));  //終了担当者
        params.add(DBUtil.stringToStringObject(getItemData(itemList, GXHDO101B017Const.BIKOU1, srSyoseiData)));  //備考1
        params.add(DBUtil.stringToStringObject(getItemData(itemList, GXHDO101B017Const.BIKOU2, srSyoseiData)));  //備考2
        if (isInsert) {
            params.add(""); //備考3
            params.add(""); //備考4
            params.add(""); //備考5
        ...
        params.add(DBUtil.stringToIntObject(getItemData(itemList, GXHDO101B017Const.TOUNYU_SETTA_MAISU, srSyoseiData)));  //投入ｾｯﾀ枚数
        params.add(getCheckBoxDbValue(getItemData(itemList, GXHDO101B017Const.SYOSEI_SETTEI_PTN_CHECK, srSyoseiData), 9));  //焼成設定ﾊﾟﾀｰﾝﾁｪｯｸ
        params.add(DBUtil.stringToIntObject(getItemData(itemList, GXHDO101B017Const.DANSU, srSyoseiData)));  //段数
        params.add(getComboValue(getItemData(itemList, GXHDO101B017Const.GAIKAN_KAKUNIN, srSyoseiData), 9));//外観確認
        params.add(DBUtil.stringToIntObject(getItemData(itemList, GXHDO101B017Const.KAISHU_SETTA_MAISU, srSyoseiData)));  //回収ｾｯﾀ枚数
        params.add(DBUtil.stringToStringObject(getItemData(itemList, GXHDO101B017Const.KAISHI_KAKUNINSYA, srSyoseiData)));  //開始確認者
        if (isInsert) {
            params.add(""); //2次脱脂号機
            params.add(0); //2次脱脂設定ﾊﾟﾀｰﾝ
            params.add(0); //2次脱脂ｷｰﾌﾟ温度
            params.add(0); //2次脱脂ｺﾝﾍﾞｱ速度
            params.add(0); //焼成ﾋﾟｰｸ温度指示
            params.add(0); //焼成ﾛｰﾗｰ速度
            params.add(0); //焼成ﾊﾟｰｼﾞ
            params.add(""); //1回目再酸化号機1
            params.add(""); //1回目再酸化号機2
            params.add(0); //1回目再酸化設定ﾊﾟﾀｰﾝ
            params.add(0); //1回目再酸化ｷｰﾌﾟ温度
            params.add(0); //1回目再酸化ｺﾝﾍﾞｱ速度
            params.add(0); //1回目再酸化後外観
        }
        params.add("焼成"); //焼成種類
    ...
    public ProcessData setStartDateTime(ProcessData processDate) {
        FXHDD01 itemDay = getItemRow(processDate.getItemList(), GXHDO101B017Const.KAISHI_DAY);
        FXHDD01 itemTime = getItemRow(processDate.getItemList(), GXHDO101B017Const.KAISHI_TIME);
    ...
    public ProcessData setEndDateTime(ProcessData processDate) {
        FXHDD01 itemDay = getItemRow(processDate.getItemList(), GXHDO101B017Const.SHURYOU_DAY);
        FXHDD01 itemTime = getItemRow(processDate.getItemList(), GXHDO101B017Const.SHURYOU_TIME);
        if (StringUtil.isEmpty(itemDay.getValue()) && StringUtil.isEmpty(itemTime.getValue())) {
            setDateTimeItem(itemDay, itemTime, new Date());
        }
