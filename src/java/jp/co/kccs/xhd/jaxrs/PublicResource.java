...
public class PublicResource {
    ...
    public Response pxhdo411(FXHDD07Json param) {
        ...
        try {
            ...
            //-----------------------------------------------FXHDD09------------------------------------------------
            //1-6.ﾒｰｶｰが"HUMO"の場合、電気特性履歴用のﾃﾞｰﾀがあるため、そちらを電気特性履歴に登録する。
            if ("HUMO".equals(param.getMaker())) {
                
                //1-7.(3)電気特性履歴のﾃﾞｰﾀを検索する。
                String sqlSelectfxhdd09 = "SELECT COUNT(lotno) cntlotno FROM fxhdd09 WHERE kojyo = ? AND lotno = ? AND edaban = ? AND deleteflag = 0 ";
                List<Object> paramsSelectfxhdd09 = new ArrayList<>(Arrays.asList(param.getKojyo(), param.getLotno(), param.getEdaban()));

                DBUtil.outputSQLLog(sqlSelectfxhdd09, paramsSelectfxhdd09.toArray(), LOGGER);
                Map resultfxhdd09 = queryRunner.query(sqlSelectfxhdd09, new MapHandler(), paramsSelectfxhdd09.toArray());

                //ｱ.取得できた場合  a.削除ﾌﾗｸﾞ=0 のﾃﾞｰﾀを更新する。   ※更新条件表(ｻｰﾊﾞ側②)ｼｰﾄ UPDATE参照
                if ((resultfxhdd09 != null) && (!"0".equals(resultfxhdd09.get("cntlotno").toString()))) {
                    // データ検索
                    String sqlfxhdd09 = "SELECT MAX(deleteflag) AS deleteflag FROM FXHDD09 WHERE kojyo = ? AND lotno = ? AND edaban = ? ";
                    List<Object> paramsfxhdd09 = new ArrayList<>(Arrays.asList(param.getKojyo(), param.getLotno(), param.getEdaban()));

                    DBUtil.outputSQLLog(sqlfxhdd09, paramsfxhdd09.toArray(), LOGGER);
                    Map fxhdd09_delflag = queryRunner.query(sqlfxhdd09, new MapHandler(), paramsfxhdd09.toArray());

                    // 削除フラグ + 1 を取得
                    int fxhdd09deleteflag = 1;
                    if (null != fxhdd09_delflag && null != fxhdd09_delflag.get("deleteflag")) {
                        fxhdd09deleteflag = (int) fxhdd09_delflag.get("deleteflag") + 1;
                    }

                    // 削除フラグ = 0 のデータが存在する場合はUPDATE
                    String sqlfxhdd09Upd = "UPDATE FXHDD09 SET koshin_date = ?, deleteflag = ? "
                            + "WHERE kojyo = ? AND lotno = ? AND edaban = ? AND deleteflag = 0";
                    List<Object> paramsfxhdd09Upd
                            = new ArrayList<>(Arrays.asList(registDate, fxhdd09deleteflag, param.getKojyo(), param.getLotno(), param.getEdaban()));

                    DBUtil.outputSQLLog(sqlfxhdd09Upd, paramsfxhdd09Upd.toArray(), LOGGER);
                    queryRunner.update(con, sqlfxhdd09Upd, paramsfxhdd09Upd.toArray());
                }

                //ｲ.上記以外の場合、電気特性履歴にﾃﾞｰﾀの登録を行う。 　※更新条件表(ｻｰﾊﾞ側②)ｼｰﾄ INSERT参照
                String kaisu[] = param.getKaisulist().split(",");
                String rirekistart[] = param.getRirekistartlist().split(",");
                String rirekiend[] = param.getRirekiendlist().split(",");
                String bin1low[] = param.getBin1lowlist().split(",");
                String bin1high[] = param.getBin1highlist().split(",");
                String bin1countersuu[] = param.getBin1countersuulist().split(",");
                
                String bin2low[] = param.getBin2lowlist().split(",");
                String bin2high[] = param.getBin2highlist().split(",");
                String bin2countersuu[] = param.getBin2countersuulist().split(",");
                
                String bin3low[] = param.getBin3lowlist().split(",");
                String bin3high[] = param.getBin3highlist().split(",");
                String bin3countersuu[] = param.getBin3countersuulist().split(",");
                
                String bin4low[] = param.getBin4lowlist().split(",");
                String bin4high[] = param.getBin4highlist().split(",");
                String bin4countersuu[] = param.getBin4countersuulist().split(",");
                
                String bin5low[] = param.getBin5lowlist().split(",");
                String bin5high[] = param.getBin5highlist().split(",");
                String bin5countersuu[] = param.getBin5countersuulist().split(",");
                
                String bin6low[] = param.getBin6lowlist().split(",");
                String bin6high[] = param.getBin6highlist().split(",");
                String bin6countersuu[] = param.getBin6countersuulist().split(",");
                
                String bin7low[] = param.getBin7lowlist().split(",");
                String bin7high[] = param.getBin7highlist().split(",");
                String bin7countersuu[] = param.getBin7countersuulist().split(",");
                
                String bin8low[] = param.getBin8lowlist().split(",");
                String bin8high[] = param.getBin8highlist().split(",");
                String bin8countersuu[] = param.getBin8countersuulist().split(",");

                // データ登録
                String sqlfxhdd09Ins = "INSERT INTO FXHDD09 ("
                        + "kojyo,lotno,edaban,kaisu,rireki_start,rireki_end,bin1low,bin1high,bin1countersuu,bin2low,bin2high,bin2countersuu,"
                        + "bin3low,bin3high,bin3countersuu,bin4low,bin4high,bin4countersuu,bin5low,bin5high,bin5countersuu,bin6low,bin6high,"
                        + "bin6countersuu,bin7low,bin7high,bin7countersuu,bin8low,bin8high,bin8countersuu,deleteflag,toroku_date,koshin_date"
                        + ") VALUES ("
                        + "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?"
                        + ")";
                ...
                String bin1lowVal = "";
                String bin1highVal = "";
                Integer bin1countersuuVal = null;
                String bin2lowVal = "";
                String bin2highVal = "";
                Integer bin2countersuuVal = null;
                
                String bin3lowVal = "";
                String bin3highVal = "";
                Integer bin3countersuuVal = null;
                
                String bin4lowVal = "";
                String bin4highVal = "";
                Integer bin4countersuuVal = null;
                
                String bin5lowVal = "";
                String bin5highVal = "";
                Integer bin5countersuuVal = null;
                
                String bin6lowVal = "";
                String bin6highVal = "";
                Integer bin6countersuuVal = null;
                
                String bin7lowVal = "";
                String bin7highVal = "";
                Integer bin7countersuuVal = null;
                
                String bin8lowVal = "";
                String bin8highVal = "";
                Integer bin8countersuuVal = null;
                
                for(int i = 0; i < kaisu.length; i++){
                    
                    if(bin1low.length - 1 < i){
                        bin1lowVal = "";
                    }else{
                        bin1lowVal = bin1low[i];
                    }
                    
                    if(bin1high.length - 1 < i){
                        bin1highVal = "";
                    }else{
                        bin1highVal = bin1high[i];
                    }
                    
                    if(bin1countersuu.length - 1 < i){
                        bin1countersuuVal = null;
                    }else{
                        bin1countersuuVal = Integer.parseInt(String.valueOf(bin1countersuu[i]));
                    }
                    
                    if(bin2low.length - 1 < i){
                        bin2lowVal = "";
                    }else{
                        bin2lowVal = bin2low[i];
                    }
                    
                    if(bin2high.length - 1 < i){
                        bin2highVal = "";
                    }else{
                        bin2highVal = bin2high[i];
                    }
                    
                    if(bin2countersuu.length - 1 < i){
                        bin2countersuuVal = null;
                    }else{
                        bin2countersuuVal = Integer.parseInt(String.valueOf(bin2countersuu[i]));
                    }
                    
                    if(bin3low.length - 1 < i){
                        bin3lowVal = "";
                    }else{
                        bin3lowVal = bin3low[i];
                    }
                    
                    if(bin3high.length - 1 < i){
                        bin3highVal = "";
                    }else{
                        bin3highVal = bin3high[i];
                    }
                    
                    if(bin3countersuu.length - 1 < i){
                        bin3countersuuVal = null;
                    }else{
                        bin3countersuuVal = Integer.parseInt(String.valueOf(bin3countersuu[i]));
                    }
                    
                    if(bin4low.length - 1 < i){
                        bin4lowVal = "";
                    }else{
                        bin4lowVal = bin4low[i];
                    }
                    
                    if(bin4high.length - 1 < i){
                        bin4highVal = "";
                    }else{
                        bin4highVal = bin4high[i];
                    }
                    
                    if(bin4countersuu.length - 1 < i){
                        bin4countersuuVal = null;
                    }else{
                        bin4countersuuVal = Integer.parseInt(String.valueOf(bin4countersuu[i]));
                    }
                    
                    if(bin5low.length - 1 < i){
                        bin5lowVal = "";
                    }else{
                        bin5lowVal = bin5low[i];
                    }
                    
                    if(bin5high.length - 1 < i){
                        bin5highVal = "";
                    }else{
                        bin5highVal = bin5high[i];
                    }
                    
                    if(bin5countersuu.length - 1 < i){
                        bin5countersuuVal = null;
                    }else{
                        bin5countersuuVal = Integer.parseInt(String.valueOf(bin5countersuu[i]));
                    }
                    
                    if(bin6low.length - 1 < i){
                        bin6lowVal = "";
                    }else{
                        bin6lowVal = bin6low[i];
                    }
                    
                    if(bin6high.length - 1 < i){
                        bin6highVal = "";
                    }else{
                        bin6highVal = bin6high[i];
                    }
                    
                    if(bin6countersuu.length - 1 < i){
                        bin6countersuuVal = null;
                    }else{
                        bin6countersuuVal = Integer.parseInt(String.valueOf(bin6countersuu[i]));
                    }
                    
                    if(bin7low.length - 1 < i){
                        bin7lowVal = "";
                    }else{
                        bin7lowVal = bin7low[i];
                    }
                    
                    if(bin7high.length - 1 < i){
                        bin7highVal = "";
                    }else{
                        bin7highVal = bin7high[i];
                    }
                    
                    if(bin7countersuu.length - 1 < i){
                        bin7countersuuVal = null;
                    }else{
                        bin7countersuuVal = Integer.parseInt(String.valueOf(bin7countersuu[i]));
                    }
                    
                    if(bin8low.length - 1 < i){
                        bin8lowVal = "";
                    }else{
                        bin8lowVal = bin8low[i];
                    }
                    
                    if(bin8high.length - 1 < i){
                        bin8highVal = "";
                    }else{
                        bin8highVal = bin8high[i];
                    }      
                    
                    if(bin8countersuu.length - 1 < i){
                        bin8countersuuVal = null;
                    }else{
                        bin8countersuuVal = Integer.parseInt(String.valueOf(bin8countersuu[i]));
                    }
                    
                    List<Object> paramfxhdd09Ins = new ArrayList<>(Arrays.asList(
                            param.getKojyo(),
                            param.getLotno(),
                            param.getEdaban(),
                            Integer.parseInt(String.valueOf(kaisu[i])),
                            new Timestamp((dtformat.parse(rirekistart[i])).getTime()),
                            new Timestamp((dtformat.parse(rirekiend[i])).getTime()),
                            bin1lowVal,
                            bin1highVal,
                            bin1countersuuVal,
                            bin2lowVal,
                    ...
                    DBUtil.outputSQLLog(sqlfxhdd09Ins, paramfxhdd09Ins.toArray(), LOGGER);
                    queryRunner.update(con, sqlfxhdd09Ins, paramfxhdd09Ins.toArray());
                }                
