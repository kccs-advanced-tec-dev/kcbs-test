...
import jp.co.kccs.xhd.model.GXHDO201B023Model;
...
@Named
@ViewScoped
public class GXHDO201B023 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B023.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B023Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B023Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B023_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B023_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            String sql ="SELECT CONCAT(T1.kojyo , T1.lotno , T1.edaban) AS lotno"
                    ...
                    + ", T2.end_ptn_dist_y5 "
                    + "FROM sr_dpprint T1 "
                    + "LEFT JOIN sub_sr_dpprint T2 ON (T1.kojyo = T2.kojyo AND T1.lotno = T2.lotno AND T1.edaban = T2.edaban AND T1.kaisuu = T2.kaisuu) "
            ...
            mapping.put("kaisuu", "kaisuu");
            mapping.put("kouteikubun", "kouteikubun");
            ...
            mapping.put("tapeatu", "tapeatu");

            BeanProcessor beanProcessor = new BeanProcessor(mapping);
            RowProcessor rowProcessor = new BasicRowProcessor(beanProcessor);
            ResultSetHandler<List<GXHDO201B023Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B023Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b023.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "印刷・DP2");

            // ダウンロードファイル名
            String downloadFileName = "印刷・DP2_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
