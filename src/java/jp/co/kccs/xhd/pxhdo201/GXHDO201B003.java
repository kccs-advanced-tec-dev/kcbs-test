...
//</editor-fold>
    ...
    public void checkInputAndSearch() {
        ...
        // ロットNo
        if (existError(validateUtil.checkC101(getLotNo(), "ロットNo", 25)) ||
    ...
    public void selectListData() {
        try {
            ...
            String sql = "SELECT CONCAT(IFNULL(KOJYO, ''), IFNULL(LOTNO, ''), IFNULL(EDABAN, '')) AS LOTNO" +
                ", EDABAN, KCPNO, TAPESYURUI, TAPELOTNO, TapeSlipKigo, GENRYOKIGO, KAISINICHIJI, SYURYONICHIJI, GOKI" +
                ", SKEEGENO, SKEEGEMAISUU, SKEEGESPEED, KANSOONDO, CLEARANCE, SAATU, MAKUATU1, SEIHANNO, SEIHANMAISUU" +
                ", PASTELOTNO, PASTENENDO, PASTEONDO, INSATUROLLNO, INSATUROLLNO2, INSATUROLLNO3, INSATUROLLNO4, INSATUROLLNO5" +
                ", INSATUHABASAVE, INSATUHABAEAVE, MLD, BIKO1, BIKO2, TANTOSYA, pkokeibun1, pastelotno2, pastenendo2" +
                ", pasteondo2, pkokeibun2, petfilmsyurui, kansoondo2, kansoondo3, kansoondo4, kansoondo5, seihanmei" +
                ", makuatsu_ave_start, makuatsu_max_start, makuatsu_min_start, makuatucv_start, nijimikasure_start" +
    ...
    private List<Object> createSearchParam() {
        ...
        if (!StringUtil.isEmpty(lotNo)) {
            ...
            paramLotNo = StringUtils.substring(getLotNo(), 3, 8);
            paramEdaban = StringUtils.substring(getLotNo(), 11, 3);
        }
        ...
        if (!StringUtil.isEmpty(startDateT)) {
            paramStartDateT = DateUtil.convertStringToDate(getStartDateT(), StringUtil.isEmpty(getStartTimeT()) ? "0000" : getStartTimeT());
        }
        ...
        if (!StringUtil.isEmpty(endDateT)) {
            paramEndDateT = DateUtil.convertStringToDate(getEndDateT(), StringUtil.isEmpty(getEndTimeT()) ? "0000" : getEndTimeT());
        }
