...
import jp.co.kccs.xhd.model.GXHDO201B009Model;
...
@Named
@ViewScoped
public class GXHDO201B009 implements Serializable {
    private static final Logger LOGGER = Logger.getLogger(GXHDO201B009.class.getName());
    ...
    /** 一覧表示データ */
    private List<GXHDO201B009Model> listData = null;
...
//<editor-fold defaultstate="collapsed" desc="#getter setter">
     ...
     * @return 一覧表示データ
     */
    public List<GXHDO201B009Model> getListData() {
        return listData;
    }
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (!StringUtil.isEmpty(selectParam.getValue("GXHDO201B009_display_page_count", session))) {
            listDisplayPageCount = Integer.parseInt(selectParam.getValue("GXHDO201B009_display_page_count", session));
        }
    ...
    public void selectListData() {
        try {
            ...
            ResultSetHandler<List<GXHDO201B009Model>> beanHandler = 
                    new BeanListHandler<>(GXHDO201B009Model.class, rowProcessor);
    ...
    public void downloadExcel() throws Throwable {
        ...
        try {
            ...
            // Excel出力定義を取得
            File file = new File(servletContext.getRealPath("/WEB-INF/classes/resources/json/gxhdo201b009.json")); 
            List<ColumnInformation> list = (new ColumnInfoParser()).parseColumnJson(file);

            // 物理ファイルを生成
            excel = ExcelExporter.outputExcel(listData, list, myParam.getString("download_temp"), "プレス・メカプレス");

            // ダウンロードファイル名
            String downloadFileName = "プレス・メカプレス_" + ((new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())) + ".xlsx";
